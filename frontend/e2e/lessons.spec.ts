...
async function startLesson(page: import('@playwright/test').Page): Promise<void> {
	...
	// Reopen the panel to drive the lesson detail view.
	await openGuideTab(page, 'Lessons');
	await expect(page.locator('.lsn-counter')).toHaveText(`Step 1 of ${LESSON.steps.length}`);
}
...
test('complete a lesson end-to-end with real move validation', async ({ page }) => {
	...
	await expect(page.locator('.lsn-counter')).toHaveText(`Step 4 of ${LESSON.steps.length}`);

	await pressMoves(page, LESSON.steps[3].expectedMoves!);
	await expect(page.locator('.lsn-status')).toHaveText('Lesson complete ✓');
});
...
test('a wrong move triggers coaching and the lesson stays completable', async ({ page }) => {
	...
	await expect(page.locator('.lsn-coaching-item.mistake')).toBeVisible();
	await expect(page.locator('.lsn-counter')).toHaveText(`Step 2 of ${LESSON.steps.length}`);

	// Recover via Back to checkpoint, then do the expected move.
	await page.locator('.lsn-btn', { hasText: /^Back to checkpoint$/ }).click();
	await settle(page);
	await pressMoves(page, LESSON.steps[1].expectedMoves!);
	await expect(page.locator('.lsn-counter')).toHaveText(`Step 3 of ${LESSON.steps.length}`);
});
...
test('Show me how opens the demo window without derailing the lesson', async ({ page }) => {
	...
	await page.locator('.lsn-btn', { hasText: /^Show me how$/ }).click();
