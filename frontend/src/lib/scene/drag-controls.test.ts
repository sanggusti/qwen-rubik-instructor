...
const { faceFromWorldNormal, dominantAxis, resolveAxisSlice, resolveDragToMove } = _internal;

function obliqueCamera(): THREE.PerspectiveCamera {
    // Mirrors the default scene pose (see CubeCanvas.svelte): an oblique view where the
    // two in-plane axes of a face foreshorten by different amounts — the situation that
...
describe('resolveAxisSlice — layer-preview guidance, independent of turn direction', () => {
    const cam = obliqueCamera();
    const X = new THREE.Vector3(1, 0, 0);
    const Y = new THREE.Vector3(0, 1, 0);

    it('agrees with resolveDragToMove on which axis/slice a drag would turn', () => {
        const p = new THREE.Vector3(1, 1.53, 0); // top face, middle row (z=0) -> S slice
        const coord = new THREE.Vector3(1, 1, 0);
        const { dx, dy } = dragAlong(cam, p, X, 40);
        const result = resolveAxisSlice(cam, pendingOn('U', p, coord), dx, dy);
        expect(result).toMatchObject({ axis: 'z', slice: 0 });
        const move = resolveDragToMove(cam, pendingOn('U', p, coord), dx, dy);
        expect(['S', "S'"]).toContain(move);
    });

    it('opposite drags along the same axis report the same layer (direction-agnostic)', () => {
        const p = new THREE.Vector3(1, 1, 1.53);
        const coord = new THREE.Vector3(1, 1, 1);
        const fwd = dragAlong(cam, p, Y, 40);
        const back = dragAlong(cam, p, Y, -40);
        const r1 = resolveAxisSlice(cam, pendingOn('F', p, coord), fwd.dx, fwd.dy);
        const r2 = resolveAxisSlice(cam, pendingOn('F', p, coord), back.dx, back.dy);
        expect(r1).toMatchObject({ axis: r2?.axis, slice: r2?.slice });
    });

    it('returns null for a drag with no in-plane component', () => {
        const p = new THREE.Vector3(1, 1, 1.53);
        const coord = new THREE.Vector3(1, 1, 1);
        const result = resolveAxisSlice(cam, pendingOn('F', p, coord), 0, 0);
